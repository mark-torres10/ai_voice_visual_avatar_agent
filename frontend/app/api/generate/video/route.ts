...
const BACKEND_VIDEO_API_URL = process.env.BACKEND_VIDEO_API_URL || 'http://localhost:3001/api/generate/video';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log('[SERVERLESS VIDEO API] Received:', { script, audioUrl, photoUrl });

    // Basic validation
    if (!script || typeof script !== 'string' || script.length < 1 || script.length > 1000) {
    ...
    const finalPhotoUrl = photoUrl && typeof photoUrl === 'string' && photoUrl.length > 0 ? photoUrl : DEFAULT_PHOTO_URL;
    ...
    if (!backendRes.ok) {
      ...
      return NextResponse.json({ error: data.error || 'Video generation failed' }, { status: backendRes.status });
    }

    // Accept both { videoUrl } and { data: { videoUrl } } shapes
    const videoUrl = data.videoUrl || data.data?.videoUrl;
    if (!videoUrl) {
      return NextResponse.json({ error: 'No videoUrl returned from backend' }, { status: 500 });
    }
